...
class BinanceRSIBot:
    ...
    def check_buy_condition(self, symbol: str, current_rsi: float) -> bool:
        """Check if RSI reaches or crosses above buy threshold (from below)"""
        previous_rsi = self.previous_rsi.get(symbol)
        if previous_rsi is not None:
            # Check if RSI crossed above or reached buy threshold (was below, now at or above)
            if previous_rsi < self.rsi_buy and current_rsi >= self.rsi_buy:
                return True
        # If we don't have previous RSI data, check if current RSI is exactly at or above threshold
        # This handles the case where a coin first appears in monitoring and is already at threshold
        elif current_rsi >= self.rsi_buy:
            # Only buy if RSI is exactly at or just crossed the threshold
            # This ensures we catch coins that reach exactly 60
            return True
        return False
    ...
    def run(self):
        ...
        if active_trade:
            ...
            # Try to detect from Binance account
            ...
            if detected_trade:
                ...
                try:
                    ...
                    if days_ago <= 30:
                        ...
                        print(f"⚠️  Detected position {detected_trade['symbol']} is {days_ago:.1f} days old - still restoring")
                        self.active_trade = detected_trade
                        active_trade = detected_trade
                        socketio.emit('active_trade_update', {'active_trade': detected_trade}, namespace='/')
                except Exception as e:
...
if __name__ == '__main__':
    ...
    # Load trade data from disk on startup
    load_trade_data()
    start_bot()
